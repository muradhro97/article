                        ...
                        <table id="dataTableExample" class="table">
                            ...
                            @foreach($articles as $item)
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge badge-warning">{{$item->user->name}}</span>
                                    </td>
                                    <td>
                                        <a style="padding: 3px;line-height: 1.5" href="{{route('admin.articles.show' , $item)}}" class="btn btn-secondary btn-icon d-inline-block">
                                            <i data-feather="eye"></i>
                                        </a>
                                        <form action="{{route('admin.articles.destroy',$item)}}" method="post" class="d-inline-block delete-form">
                                            @csrf
                                            @method('DELETE')
